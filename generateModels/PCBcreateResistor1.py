...
__fcstdFile__ = "resistor1THT.FCStd"
__desc__ = "Resistor 1 THT"


class modelPreview(modelPreviewMain):
     def __init__(self, parent=None):
        modelPreviewMain.__init__(self, "resistor1.png", __desc__, parent)
    ...
    def checkParam(self, dummy):
        ...
        try:
            if self.len_1.value() >= self.len_4.value():
                self.errors = True
                self.errorsList.setText("Error: r is smaller (or equal) than w\n")
        except:
            pass
...
def modelGenerate(doc, widget):
    ...
    doc.Spreadsheet.set('B5', str(widget.len_4.value()))
    doc.Spreadsheet.set('B4', str(widget.diameter.value()))
    doc.recompute()
